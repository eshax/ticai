                        ...
                        <div class="name-with-board">
                          <span class="board-indicator">{{ stock.boardIndicator }}</span>
                            <el-tooltip placement="top">
                              <template #content> {{ stock.surge_reason.stock_reason }} </template>
...
const formatStockData = (stock) => {
  ...
  return {
    ...
    originalPlateName: '',
    surge_reason: stock.surge_reason || {}, // 保留涨停原因相关数据
    wasUpdated: false,
...
/* 股票列 - 减少底部外边距和内边距 */
.stock-col {
  margin-bottom: 6px !important; /* 进一步减小底部间距 */
  padding-left: 4px !important;  /* 减小左侧内边距 */
...
.stock-card {
  ...
  width: 100%;
  cursor: default; /* 修改为默认光标，因为已无详情弹窗 */
  margin: 0 !important; /* 移除卡片自身的外边距 */
}

/* 调整卡片内部边距 */
:deep(.el-card__header) {
  padding: 6px !important; /* 减小卡片头部内边距 */
}

:deep(.el-card__body) {
  padding: 0px !important; /* 减小卡片主体内边距 */
}
...
:deep(.el-descriptions__cell) {
  border-color: #eee !important;
  padding: 2px 8px !important; /* 减小单元格内边距 */
}
...
.el-descriptions__item {
  padding: 2px 0 !important; /* 减小描述项间距 */
}

.el-descriptions__label {
  font-size: 11px !important; /* 减小标签字体 */
  padding-right: 6px !important;
  color: #666 !important;
}

.el-descriptions__content {
  font-size: 11px !important; /* 减小内容字体 */
  color: #333 !important;
}
