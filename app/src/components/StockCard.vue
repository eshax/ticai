...
const getStockChanges = (oldStock, newStock) => {
  ...
  // 只检查这些字段的变化，忽略价格变化
  const compareFields = [
    { field: 'change_percent', label: '涨跌幅' }, // 保留涨跌幅，因为涨停状态变化会影响这个值
...
const formatStockData = (stock, isDown = false) => {
  ...
  return {
    ...
    price: stock.price,  // 保留价格字段，但不触发提示
